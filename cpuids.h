	...
	union {
		struct {
			...
			struct {
				...
				uint32_t LogicalProcessors : 8;	// [16:23] Logical processors count
				uint32_t LocalAPIC_ID : 8;	// [24:31] Local APIC ID — this number is the 8-bit ID that is assigned to the local APIC on the processor during power up. This field was introduced in the Pentium 4 processor.
			} EBX;
			struct {
				...
				uint32_t VMGUEST : 1;		// VMGUEST - Inside virtual machine (VM Guest)
			} ECX;
			struct {
