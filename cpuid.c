...
{
	uint32_t __ext = __leaf & 0x80000000;
...
CPU_FMS * __cdecl cpu_fms() {
	...
	fms.family = ft->EAX.Family;
	fms.model = ft->EAX.Model;
	fms.stepping = ft->EAX.Stepping;
	if (fms.family == 0x06 || fms.family == 0x0F)
		fms.model |= (ft->EAX.ExtendedModel << 4);
	if (fms.family == 0x0F)
		fms.family += ft->EAX.ExtendedFamily;
	return &fms;
}
